...
const ChatPage = () => {
    ...
    return (
        ...
        <div className='flex h-screen md:ml-[16%]'>
            <section className=' md:w-1/2 my-6'>
                <h1 className='font-bold mb-1 px-3 text-xl'>{user?.username?.split(' ')[0]}</h1>
                        ...
                        suggestedUsers.map((suggestedUser) => {
                            ...
                            return (
                                <div onClick={() => dispatch(setSelectedUser(suggestedUser))} className='flex gap-3 items-center p-3 hover:bg-gray-50 cursor-pointer'>
                                    ...
                                    <span className='font-medium'> {suggestedUser?.username?.split(' ')[0]} </span>                                        <span className={`text-xs font-bold ${isOnline ? 'text-green-600' : 'text-red-600'} `}>{isOnline ? 'online' : 'offline'}</span>
                                    </div>
                                </div>
                            )
                        })
            ...
            </section>
            {
                selectedUser ? (
                    <section className='flex-1 border-l border-l-gray-300 flex flex-col h-full'>
                        ...
                        <div className='flex mb-10 items-center p-4 border-t border-t-gray-300'>
